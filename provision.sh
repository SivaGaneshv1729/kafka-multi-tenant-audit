...
for tenant in "${TENANTS[@]}"; do
    ...
    docker exec multi-tenant-audit-log-kafka-1 kafka-configs --bootstrap-server $BOOTSTRAP_SERVER --command-config /tmp/admin.conf --alter --add-config "SCRAM-SHA-256=[password=$tenant-password]" --entity-type users --entity-name $tenant
